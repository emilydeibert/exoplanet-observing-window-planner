...
### 2025A SEMESTER ###
start_of_semester = gemini.twilight_evening_nautical(Time('2025-12-01T00:00:00', format='isot', scale='utc'), which='next')
end_of_semester = gemini.twilight_morning_nautical(Time('2026-02-01T00:00:00', format='isot', scale='utc'), which='next')
...
transit_baseline = 45. * u.minute # on either side of the transit
...
def plot_single_transit_observation(planet_params, star, site, ing_egr_list, flexible_list, baseline, overhead, exp_time):
	...
	occult = occult_eclipse_model(0, 1, planet_params['pl_orbincl'].value, planet_params['pl_ratror'], ratdor,
...
def find_eclipse_windows(start_time, end_time, planet_params, star, phase_lim, max_airmass, eclipseLength, site = gemini):
	...
	if planet_params['pl_name'] == 'WASP-103 b':
		...
		occult = occult_eclipse_model(0, 1, planet_params['pl_orbincl'].value, planet_params['pl_ratror'].value, planet_params['pl_ratdor'],
	...
	while start < end_time:
		# check each day...probably not the most efficient but not sure what else to do
...
def plot_single_eclipse_observation(planet_params, star, site, eclipse_window, overhead, exp_time, eclipseLength):
	...
	occult = occult_eclipse_model(0, 1, planet_params['pl_orbincl'].value, planet_params['pl_ratror'], planet_params['pl_ratdor'],
